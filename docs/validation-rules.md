...
```bash
make check-sanity FILES="${FILE_PATH}"
```
...
### Quick Check
```bash
make check-sanity FILES="./path/to/file.go"
```

### Detailed Report
```bash
make report-sanity FILES="./path/to/file.go"
```
...
make check-sanity FILES="./path/to/file_test.go"

# CORRECT - Target the package instead
make check-sanity FILES="./path/to/package"

# CORRECT - Target both source and test files together
make check-sanity FILES="./path/to/file.go ./path/to/file_test.go"
```
...
make report-sanity
# Or file-only:
make report-sanity FILES="${FILE_PATH}"
```
...
### Fix Single File
```bash
make check-sanity FILES="./cmd/validate/image.go"
make report-sanity FILES="./cmd/validate/image.go"
```
...
|-------------|---------|-------------|
| Single file edit | `make check-sanity FILES="${FILE_PATH}"` | Small changes to one file |
| Multi-file change | `make check-sanity` | Multiple files, no major refactor |
| Major refactor | `make refactor && make test` | Structural changes, renames, deletions |
| Behavior change | `make behavior` | Logic changes, algorithm modifications |
| Pre-commit | `make prepr` | Before PR or review |
